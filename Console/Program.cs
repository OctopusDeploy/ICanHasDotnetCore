        ...
        private static IEnumerable<SourcePackageFile> FindFiles(IEnumerable<string> directories)
        {
            foreach (var directory in directories.Where(d => !ExcludeDirectories.Contains(Path.GetFileName(d))))
            {
                ...
                if (configFile.Exists)
                    yield return new SourcePackageFile(configFile.DirectoryName, File.ReadAllBytes(configFile.FullName));

                var jsonFile = new FileInfo(Path.Combine(directory, "project.json"));
                if (jsonFile.Exists)
                    yield return new SourcePackageFile(jsonFile.DirectoryName, File.ReadAllBytes(jsonFile.FullName));
